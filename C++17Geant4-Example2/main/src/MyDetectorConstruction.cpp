...
//	labMaterial = std::unique_ptr<G4Material>(air);
	labMaterial = air;
}
...
G4VPhysicalVolume* MyDetectorConstruction::ConstructDetector()
{
	...
	logicalLab = std::make_unique<G4LogicalVolume>(solidLab.get(), //raw pointer
//												labMaterial.get(), //raw pointer
												labMaterial, 	   //raw pointer
												"Lab");

	physicalLab =
		std::make_unique<G4PVPlacement>(nullptr,	   	 //no rotation
	...
	return physicalLab.get(); //raw pointer as smart pointer not accepted
}
